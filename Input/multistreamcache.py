...
class MultistreamCache():
    def __init__(self,
        ...
        self.communication_queue = Queue(maxsize=50)  #hardcoded for now
        self.worker_handles = []
    ...
    def update_cache_from_queue(self):
        ...
        while True:
            ...
            if (average_replacement_rate_updated >= self.min_replacement_rate): #N.B. only have additional condition if we actually want to use all available data samples everytime. Incorporate self.communication_queue.empty()) ?
                break
